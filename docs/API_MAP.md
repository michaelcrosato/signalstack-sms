...
- `/dashboard/campaigns`: renders a product-facing campaign workspace with local draft creation, compliance preflight, local schedule actions, recipient readiness, and delivery review status backed by existing campaign APIs without provider calls, SMS, billing, live AI, secrets, worker execution, or live messaging enablement.
