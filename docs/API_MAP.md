...
- `/dashboard`: renders a product-facing dashboard with tenant-scoped product metrics, outbound-only local message delivery rate/pending/failure/review signals, local usage totals, and navigation links without mutations, delivery retries, provider calls, SMS, billing artifacts, live AI, secrets, or live messaging enablement.
