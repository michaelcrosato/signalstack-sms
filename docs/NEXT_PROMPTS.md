...
- Latest validated run: Run 789 tightens campaign preflight so missing or cross-tenant requested contact IDs are blocked as `CONTACT_NOT_FOUND`, and the product composer now renders the API's allowed/blocked preflight counts.
