...
- Post-MVP local operations inventory hardening: `/settings/operations` now uses a shared local operator surface inventory with unit coverage for grouped app routes, duplicate route prevention, backing `app/**/page.tsx` files, current surface counts, and safety-sensitive operations links before seeded browser coverage.
- Post-MVP local API operations inventory hardening: `/settings/api` route inventory unit coverage now verifies unique route-method rows, backing `app/**/route.ts` files for listed API paths, current local route count, and zero external-impact routes before browser demo coverage.
