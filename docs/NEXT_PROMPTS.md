...
- Latest validated run: Run 791 makes local outbound worker idempotency keys tenant-explicit before dummy provider calls and message upserts.
- Run 790 centralized local outbound delivery count filters so dashboard and analytics use the same total/delivered/pending/failed query shapes.
- Run 789 tightened campaign preflight so missing or cross-tenant requested contact IDs are blocked as `CONTACT_NOT_FOUND`, and the product composer renders the API's allowed/blocked preflight counts.
