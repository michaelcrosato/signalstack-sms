...
- The browser product now has an initial `/dashboard` shell, `/dashboard/contacts` list/import workflow, `/dashboard/contacts/:contactId` local detail/edit workflow, `/dashboard/campaigns` compose/preflight/local schedule workflow, `/dashboard/inbox` local thread workflow, `/dashboard/templates` local template create/list workflow, and `/dashboard/compliance` local readiness detail.
...
1. Add product-facing analytics detail on existing local analytics APIs.
