...
- Twilio webhook normalization now trims inbound `From`/`To` addresses before local contact/message creation, rejects whitespace-only inbound bodies without trimming stored nonblank body text, trims provider message IDs and error codes, normalizes delivery-status casing/whitespace before local idempotency keys are derived, and clears stale opposite terminal timestamps for delivered versus failed/undelivered transitions, so retry formatting drift and local delivery metadata do not create confusing duplicate or stale states.
