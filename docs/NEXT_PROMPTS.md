...
- Latest validated run: Run 788 adds direct Twilio webhook route coverage for invalid form/signature fail-closed behavior, duplicate-event no-op behavior, and tenant-scoped status updates.
...
- Twilio webhook handlers fail closed on malformed or unsupported form bodies before signature validation, tenant lookup, webhook-event storage, or local inbox/delivery mutations; route-level tests now also pin invalid signatures, duplicate-event no-ops, and current-tenant status updates.
