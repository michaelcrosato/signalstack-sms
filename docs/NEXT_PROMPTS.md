...
- API operations implemented-route reverse coverage now reuses the shared contract route-method extractor, so named-export, typed const, and non-code-masked route method detection stays aligned with `npm run contracts:check`.
