...
- Latest validated run: Run 784 adds owner-facing pending outbound delivery visibility to campaign detail and analytics from local message rows only.
...
- Twilio webhook handlers fail closed on malformed or unsupported form bodies before signature validation, tenant lookup, webhook-event storage, or local inbox/delivery mutations.
