...
The demo operations screen at `/settings/demo` displays seeded demo readiness, workflow links, local metrics, usage totals, and runtime gates. It is read-only and does not import contacts, schedule or cancel campaigns, run workers, create inbox replies, submit prompts, execute reports, create exports, mutate records, enqueue jobs, call Redis, call providers, bill, notify, expose secrets, or enable live features.
