...
| Tests/Gates | `npm run validate`, unit tests, Playwright smoke/demo, contract/secret/compliance/production gates. | Browser smoke and demo path; product dashboard navigation has unit coverage. | Product-demo path does not yet cover real owner workflow. | Add `e2e/product-demo-path.spec.ts` for the dashboard workflow. |
