...
| --- | --- | --- | --- | --- |
| Contacts | Tenant-scoped contacts, consent fields, CSV import, tags/lists/segments foundations. | Product dashboard summary plus read-only operations views and demo path coverage. | No polished product list/import/edit workflow. | Build product contacts page on existing APIs. |
...
| Tests/Gates | `npm run validate`, unit tests, Playwright smoke/demo/product-demo, contract/secret/compliance/production gates. | Browser smoke, seeded investor demo path, and seeded product dashboard path. | Deeper product flows still need UI coverage as they are built. | Expand product E2E with contacts, campaigns, inbox, and templates workflows. |
