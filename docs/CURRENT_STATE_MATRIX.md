...
| Campaigns | Drafts, recipients, preflight, schedule/cancel records, scheduled-only local cancellation with conflict responses for existing non-scheduled campaigns, queue jobs, dummy worker path. | Product dashboard summary plus `/dashboard/campaigns` composer with fake-AI copy assist, recipient selection, frozen campaign metric-row and composer-default metadata, preflight, local schedule, status table, `/dashboard/campaigns/:campaignId` draft edit/queued-cancel workflow, and product demo coverage. | No advanced campaign history or send-result drilldown beyond local lifecycle state. | Keep live sends blocked; add deeper reporting only after product demo feedback. |
