...
| Campaigns | Drafts, recipients, preflight, schedule/cancel records, queue jobs, dummy worker path. | Product dashboard summary plus `/dashboard/campaigns` composer, recipient selection, preflight, local schedule, status table, and product demo coverage. | No campaign detail/edit/cancel flow after initial scheduling. | Build inbox and template product flows before deeper campaign editing. |
