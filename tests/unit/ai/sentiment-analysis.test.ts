...
import { createDemoInboundMessage, createConversationInboundMessage } from "@/lib/db/repositories/inbox";
import { prisma } from "@/lib/db/prisma";

const messages: AiMessage[] = [
  { direction: "INBOUND", body: "Hello, I am interested in pricing and options" },
...
describe("Conversation Sentiment and AI Categorization Seam", () => {
  describe("fakeConversationSentiment", () => {
    ...
    it("identifies support/help keywords as SUPPORT and NEUTRAL", () => {
      const supportMessages: AiMessage[] = [
        { direction: "INBOUND", body: "Please help me with setting up my system" }
      ];
  ...
  describe("liveAiProvider.analyzeConversationSentiment", () => {
    it("correctly calls Anthropic API and parses JSON sentiment analysis", async () => {
      const fetchMock = vi.fn().mockResolvedValue({
        ok: true,
        json: async () => ({
          content: [{ text: '{"sentiment": "POSITIVE", "category": "INQUIRY"}' }]
        })
      });
      vi.stubGlobal("fetch", fetchMock);
      vi.stubEnv("AI_API_KEY", "test-key");

      const result = await liveAiProvider.analyzeConversationSentiment({ messages });
      expect(result).toEqual({
        provider: "live",
        sentiment: "POSITIVE",
        category: "INQUIRY"
    ...
    it("throws on an unparseable json response from live AI", async () => {
      ...
      await expect(liveAiProvider.analyzeConversationSentiment({ messages }))
        .rejects.toThrow("Live AI returned an unparseable conversation sentiment.");
    });
  });
  ...
  describe("database integration", () => {
    ...
    it("async triggers and updates conversation sentiment/category in createConversationInboundMessage", async () => {
      const org = await prisma.organization.create({
        data: { slug: `org-sentiment-conv-${Date.now()}`, name: `Org Sentiment Conv-${Date.now()}`, demoMode: true }
      });
      ...
      const conversation = await prisma.conversation.create({
        data: { orgId: org.id, contactId: contact.id, status: ConversationStatus.OPEN }
      });
