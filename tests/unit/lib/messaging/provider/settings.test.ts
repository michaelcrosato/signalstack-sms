...
describe("getProviderSettings", () => {
  ...
  it("should redact twilio credentials configured via environment", () => {
    const input: ProviderSettingsInput = {
      ...baseInput,
      env: {
        TWILIO_ACCOUNT_SID: "AC1234567890",
        TWILIO_AUTH_TOKEN: "supersecrettoken",
        TWILIO_FROM_NUMBER: "+12345678901"
      }
    };

    const result = getProviderSettings(input);
    expect(result.twilio.accountSidRedacted).toBe("********7890");
    expect(result.twilio.fromNumberRedacted).toBe("********8901");
  });
});
