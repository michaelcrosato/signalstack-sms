...
function routeToAppPagePath(route: string) {
  ...
  return join(process.cwd(), "src", "app", ...routeSegments, "page.tsx");
}
  ...
  function walk(directory: string) {
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        const relativeDirectory = relative(join(process.cwd(), "src", "app"), directory);
...
describe("operator surface inventory", () => {
  ...
  it("lists every implemented local operator page in the shared inventory", () => {
    const implementedOperatorRoutes = [...collectPageRoutes(join(process.cwd(), "src", "app", "settings")), "/demo"].sort();
