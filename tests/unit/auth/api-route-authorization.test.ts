...
function bodySliceParsesRequestBody(bodySlice: string, requestParameterName = defaultRequestParameterName) {
  ...
  const reflectedBodySlice = bodySlice
    ...
    .replace(
      new RegExp(
        `Object\\s*\\.\\s*getOwnPropertyDescriptor\\s*\\(\\s*Request\\s*\\.\\s*prototype\\s*,\\s*["'\`](${requestBodyReaderNames})["'\`]\\s*\\)\\s*\\??\\.\\s*value`,
        "g"
      ),
      "Request.prototype.$1"
    )
    .replace(
      new RegExp(
        `Object\\s*\\.\\s*getOwnPropertyDescriptor\\s*\\(\\s*Request\\s*\\.\\s*prototype\\s*,\\s*([A-Za-z_$][\\w$]*)\\s*\\)\\s*\\??\\.\\s*value`,
        "g"
      ),
  ...
  const prototypeReaderAliasPattern = new RegExp(
    `${variableDeclaratorStart}([A-Za-z_$][\\w$]*)\\s*(?::[^=;,\\n]+)?\\s*=\\s*Request\\s*\\.\\s*prototype\\s*\\.\\s*(?:${requestBodyReaderNames})${variableDeclaratorEnd}`,
    "g"
  );
  const assignedPrototypeReaderAliasPattern = new RegExp(
    `(?:^|[;\\r\\n])\\s*([A-Za-z_$][\\w$]*)\\s*=\\s*Request\\s*\\.\\s*prototype\\s*\\.\\s*(?:${requestBodyReaderNames})\\s*(?=;|\\r?\\n)`,
    "g"
  );
