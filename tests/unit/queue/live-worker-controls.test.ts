...
describe("production live campaign worker controls", () => {
  ...
  it("rejects object-valued control-entry metadata without coercing it", () => {
    const implementedControls = implementedFrozenControls();

    for (const metadataName of [Symbol.toStringTag, Symbol.toPrimitive, "toString", "valueOf"] as const) {
      const metadataObject = Object.freeze({
        [Symbol.toPrimitive]: () => {
          throw new Error(`control-entry ${String(metadataName)} metadata object must not use Symbol.toPrimitive`);
        },
        toString: () => {
          throw new Error(`control-entry ${String(metadataName)} metadata object must not use toString`);
        },
        valueOf: () => {
          throw new Error(`control-entry ${String(metadataName)} metadata object must not use valueOf`);
        }
      });

      const metadataControls = Object.freeze([
        Object.freeze(
          Object.defineProperty({ ...implementedControls[0] }, metadataName, {
            enumerable: false,
  ...
  it("rejects object-valued deployment class metadata without coercing it or inspecting controls", () => {
    const throwingEvidence = new Proxy([...implementedFrozenControls()], {
      getPrototypeOf: () => {
        throw new Error("object-valued deployment class metadata must not inspect control evidence");
      },
      getOwnPropertyDescriptor: () => {
        throw new Error("object-valued deployment class metadata must not inspect control evidence");
      },
    ...
    const hostileMetadataValue = (metadataName: PropertyKey) =>
      Object.freeze({
        [Symbol.toPrimitive]: () => {
          metadataCoerced = true;
          throw new Error(`deployment class ${String(metadataName)} metadata must not use Symbol.toPrimitive`);
        },
        toString: () => {
          metadataCoerced = true;
          throw new Error(`deployment class ${String(metadataName)} metadata must not use toString`);
        },
        valueOf: () => {
          metadataCoerced = true;
          throw new Error(`deployment class ${String(metadataName)} metadata must not use valueOf`);
        }
      });
    const metadataKeys = [
      Symbol.toStringTag,
  ...
  it("rejects accessor-backed deployment class metadata without reading it or inspecting controls", () => {
    const throwingEvidence = new Proxy([...implementedFrozenControls()], {
      getPrototypeOf: () => {
        throw new Error("accessor-backed deployment class metadata must not inspect control evidence");
      },
      getOwnPropertyDescriptor: () => {
        throw new Error("accessor-backed deployment class metadata must not inspect control evidence");
      },
    ...
    for (const metadataName of metadataKeys) {
      ...
      Object.defineProperty(workerDeploymentClass, metadataName, {
        ...
        get: () => {
          metadataRead = true;
          throw new Error(`deployment class ${String(metadataName)} metadata getter must not be read`);
        }
      });
      Object.freeze(workerDeploymentClass);
  ...
  it("rejects hidden deployment class metadata without reading, invoking, coercing it or inspecting controls", () => {
    const throwingEvidence = new Proxy([...implementedFrozenControls()], {
      getPrototypeOf: () => {
        throw new Error("hidden deployment class metadata must not inspect control evidence");
      },
      getOwnPropertyDescriptor: () => {
        throw new Error("hidden deployment class metadata must not inspect control evidence");
      },
    ...
    const hiddenSymbolMetadata = Symbol("hidden-live-worker-deployment-class");
    let metadataRead = false;
    let metadataInvoked = false;
    let metadataCoerced = false;
    const hostileObjectMetadata = (metadataName: PropertyKey) =>
      Object.freeze({
        [Symbol.toPrimitive]: () => {
          metadataCoerced = true;
          throw new Error(`hidden deployment class ${String(metadataName)} metadata must not use Symbol.toPrimitive`);
        },
        toString: () => {
          metadataCoerced = true;
          throw new Error(`hidden deployment class ${String(metadataName)} metadata must not use toString`);
        },
        valueOf: () => {
          metadataCoerced = true;
          throw new Error(`hidden deployment class ${String(metadataName)} metadata must not use valueOf`);
        }
      });
    const accessorMetadataClass = Object.freeze(
      Object.defineProperties(
        {},
        {
          reviewerBypass: {
            ...
            get: () => {
              metadataRead = true;
              throw new Error("hidden deployment class string metadata getter must not be read");
            }
          },
          [hiddenSymbolMetadata]: {
            enumerable: false,
            get: () => {
              metadataRead = true;
              throw new Error("hidden deployment class symbol metadata getter must not be read");
            }
          }
        }
      )
    );
    const callableMetadataClass = Object.freeze(
      Object.defineProperties(
        {},
        {
          reviewerBypass: {
            ...
            value: () => {
              metadataInvoked = true;
              throw new Error("hidden deployment class string metadata must not be invoked");
            }
          },
          [hiddenSymbolMetadata]: {
            enumerable: false,
            value: () => {
              metadataInvoked = true;
              throw new Error("hidden deployment class symbol metadata must not be invoked");
            }
          }
        }
      )
    );
    const objectMetadataClass = Object.freeze(
      Object.defineProperties(
        {},
        {
          reviewerBypass: {
  ...
  it("rejects inherited accessor-backed deployment class metadata without reading it or inspecting controls", () => {
    const throwingEvidence = new Proxy([...implementedFrozenControls()], {
      getPrototypeOf: () => {
        throw new Error("inherited accessor-backed deployment class metadata must not inspect control evidence");
      },
      getOwnPropertyDescriptor: () => {
        throw new Error("inherited accessor-backed deployment class metadata must not inspect control evidence");
      },
    ...
    for (const metadataName of metadataKeys) {
      ...
      Object.defineProperty(deploymentClassPrototype, metadataName, {
        ...
        get: () => {
          metadataRead = true;
          throw new Error(`inherited deployment class ${String(metadataName)} metadata getter must not be read`);
        }
      });
      const workerDeploymentClass = Object.freeze(Object.create(deploymentClassPrototype));
  ...
  it("rejects inherited data-backed callable deployment class metadata without invoking it or inspecting controls", () => {
    const throwingEvidence = new Proxy([...implementedFrozenControls()], {
      getPrototypeOf: () => {
        throw new Error("inherited data-backed callable deployment class metadata must not inspect control evidence");
      },
      getOwnPropertyDescriptor: () => {
        throw new Error("inherited data-backed callable deployment class metadata must not inspect control evidence");
      },
    ...
    for (const metadataName of metadataKeys) {
      ...
      Object.defineProperty(deploymentClassPrototype, metadataName, {
        ...
        value: () => {
          metadataInvoked = true;
          throw new Error(`inherited deployment class ${String(metadataName)} callable metadata must not be invoked`);
        }
      });
      const workerDeploymentClass = Object.freeze(Object.create(deploymentClassPrototype));
  ...
  it("rejects object-valued authorization-wrapper constructor or toLocaleString metadata without coercing it or inspecting controls", () => {
    const throwingEvidence = new Proxy(implementedFrozenControls(), {
      getPrototypeOf: () => {
        throw new Error("object-valued wrapper legacy metadata must not inspect control evidence");
      },
      getOwnPropertyDescriptor: () => {
        throw new Error("object-valued wrapper legacy metadata must not inspect control evidence");
      },
    ...
    for (const metadataName of ["constructor", "toLocaleString"] as const) {
      const wrapper = {
        workerDeploymentClass: reservedLiveWorkerDeploymentClass,
        controls: throwingEvidence
      } as Record<PropertyKey, unknown>;
