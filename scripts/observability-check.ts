...
const resolvePath = (file: string) => existsSync(file) ? file : (existsSync(`src/${file}`) ? `src/${file}` : file);
const instrumentation = readFileSync(resolvePath("instrumentation.ts"), "utf8");
const observabilityLogger = readFileSync(resolvePath("lib/observability/logger.ts"), "utf8");
const observabilityMetrics = readFileSync(resolvePath("lib/observability/metrics.ts"), "utf8");
