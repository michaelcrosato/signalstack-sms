...
export async function POST(request: Request) {
  ...
  try {
    ...
    const rawSessionToken = readLocalSessionToken(request, {
    ...
    const membership = await selectOrganizationForSession(
      { userId: currentOrg.userId },
      rawSessionToken,
      rawPayload
    );
    return noStoreJson(membership, 200);
  } catch (error) {
...
function authenticationRequiredResponse() {
  return noStoreJson({ error: "Authentication required.", code: "AUTH_REQUIRED" }, 401);
}
