...
import { listProviderMessagingServices } from "@/lib/integrations/provider-accounts/service";
import {
  noStoreResponse,
  providerJson,
  providerRouteError,
...
export async function GET(_request: Request, context: AccountRouteContext) {
  ...
  try {
    const messagingServices = await listProviderMessagingServices(
      currentOrg.orgId,
      routeParam(accountId)
    );
    return providerJson({ messagingServices });
  } catch (error) {
