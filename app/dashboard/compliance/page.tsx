          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div className="space-y-2">
              ...
              <p className="max-w-3xl text-base leading-7 text-slate-700">
                Review the profile fields, A2P status, and runtime hard-gate
                blockers that keep campaign sending demo-safe until explicit
                go-live work is complete.
              </p>
            </div>
            <Link
      ...
      <div className="mx-auto grid w-full max-w-7xl gap-6 px-6 py-6">
        ...
        <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <Panel title="Profile Checklist">
            <ul className="grid gap-3">
              {compliance.fields.map((field) => (
                <li
                  className="grid gap-1 border-b border-slate-100 pb-3"
                  key={field.key}
                >
          ...
          <Panel title="Runtime Gates">
            <dl className="grid gap-3 text-sm">
              <StatusRow
                label="Demo mode"
                value={String(compliance.summary.demoMode)}
              />
              <StatusRow
                label="Live messaging flag"
                value={String(compliance.summary.liveMessagingEnabled)}
              />
              <StatusRow
                label="Messaging provider"
                value={compliance.summary.messagingProvider}
              />
              <StatusRow
                label="Live messaging allowed"
                value={String(compliance.summary.liveMessagingAllowed)}
              />
              <StatusRow
                label="Updated"
                value={compliance.profile.updatedAt.toLocaleString("en-US")}
              />
            </dl>
          </Panel>
        </section>
          ...
          <ul className="grid gap-3 text-sm">
            {compliance.blockers.length > 0 ? (
              ...
              <li className="text-slate-700">
                No hard-gate blockers recorded.
              </li>
            )}
          </ul>
        </Panel>
        ...
        <Panel title="Safety Boundary">
          <ul className="grid gap-2 text-sm text-slate-700">
            <li>
              This workspace reads local compliance readiness and runtime gate
              state.
            </li>
            <li>
              It does not register A2P campaigns, call providers, send SMS,
              create billing records, or enable live flags.
            </li>
            <li>
              Go-live remains controlled by protected validation gates and
              explicit production readiness work.
            </li>
          </ul>
        </Panel>
      </div>
