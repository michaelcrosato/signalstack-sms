...
export default async function ContactDetailPage({
  ...
  const [{ contactId }, currentOrg] = await Promise.all([
    params,
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div className="space-y-2">
              ...
              <p className="max-w-3xl text-base leading-7 text-slate-700">
                Update local profile, consent evidence, notes, tags, and lists
                before using this contact in campaign preflight.
              </p>
            </div>
            <div className="flex flex-wrap gap-2 text-sm">
              <Link
                className="rounded border border-slate-300 px-3 py-2 font-medium text-slate-700"
      ...
      <div className="mx-auto grid w-full max-w-7xl gap-6 px-6 py-6 lg:grid-cols-[1fr_0.55fr]">
        <section className="rounded border border-slate-200 bg-white p-5">
          ...
          <p className="mt-1 text-sm text-slate-600">
            Changes use the existing tenant-scoped contact API. Archive is a
            soft archive only.
          </p>
          <div className="mt-5">
        ...
        <aside className="grid content-start gap-6">
          ...
          <section className="rounded border border-slate-200 bg-white p-5">
            ...
            <p className="mt-3 text-sm leading-6 text-slate-700">
              This page mutates local contact metadata only. It does not send
              SMS, call providers, create billing records, call live AI, expose
              secrets, hard-delete contacts, bypass consent checks, or enable
              live messaging.
            </p>
          </section>
        </aside>
...
function StatusRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-4 border-b border-slate-100 pb-2">
      ...
      <dd className="max-w-52 text-right font-medium text-slate-950">
        {value}
      </dd>
    </div>
  );
