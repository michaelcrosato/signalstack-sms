        ...
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-5 px-6 py-6">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <Link className="rounded border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700" href="/dashboard/templates">
                Templates
              </Link>
      ...
      <div className="mx-auto grid w-full max-w-7xl gap-6 px-6 py-6">
        ...
        <section className="grid gap-6 lg:grid-cols-[1fr_0.8fr]">
          <section className="rounded border border-slate-200 bg-white p-5">
            ...
            <p className="mt-1 text-sm text-slate-600">Edits update the tenant-scoped local template row.</p>
            <div className="mt-5">
          ...
          <section className="rounded border border-slate-200 bg-white p-5">
            ...
            <dl className="mt-4 grid gap-3 text-sm">
              <div className="rounded border border-slate-200 bg-slate-50 p-3">
                <dt className="font-semibold text-slate-950">Campaigns using this template</dt>
                <dd className="mt-1 text-slate-700">{template.campaignUsage}</dd>
              </div>
              <div className="rounded border border-slate-200 bg-slate-50 p-3">
                <dt className="font-semibold text-slate-950">Variable names</dt>
                <dd className="mt-1 text-slate-700">{template.variables.length > 0 ? template.variables.join(", ") : "none"}</dd>
              </div>
            </dl>
          </section>
        </section>
