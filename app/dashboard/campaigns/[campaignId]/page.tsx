...
export default async function CampaignDetailPage({
  ...
  const [{ campaignId }, currentOrg] = await Promise.all([
    params,
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div className="space-y-2">
              ...
              <p className="max-w-3xl text-base leading-7 text-slate-700">
                Review campaign copy, edit draft recipients, and cancel queued
                local work without provider sends.
              </p>
            </div>
      ...
      <div className="mx-auto grid w-full max-w-7xl gap-6 px-6 py-6">
        ...
        <section className="grid gap-6 lg:grid-cols-[1fr_0.8fr]">
          <section className="rounded border border-slate-200 bg-white p-5">
            ...
            <p className="mt-1 text-sm text-slate-600">
              Drafts can be edited. Scheduled campaigns can be canceled locally.
            </p>
            <div className="mt-5">
          ...
          <section className="rounded border border-slate-200 bg-white">
            <div className="border-b border-slate-200 p-5">
              ...
              <p className="mt-1 text-sm text-slate-600">
                Current campaign recipients and send eligibility signals.
              </p>
            </div>
            <div className="grid gap-3 p-5">
              <section
                aria-label="Recipient readiness summary"
        ...
        <section
          aria-label="Campaign delivery snapshot"
          className="rounded border border-slate-200 bg-white"
        >
          <div className="border-b border-slate-200 p-5">
            <h2 className="text-xl font-semibold">Delivery snapshot</h2>
            <p className="mt-1 text-sm text-slate-600">
              Metrics summarize all outbound local messages; recent rows show
              the latest 30 evidence records.
            </p>
          </div>
            ...
            <div className="grid gap-3">
              <div>
                <h3 className="text-sm font-semibold text-slate-800">
                  Recent outbound evidence
                </h3>
                <p className="mt-1 text-sm text-slate-600">
                  These rows are local ledger records only and do not trigger
                  delivery retries or provider calls.
                </p>
              </div>
                  ...
                  >
                    <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-slate-600">
                          {message.direction} / {message.deliveryState} /{" "}
                          {message.providerStatus} / {message.providerMessageId}
                        </p>
                      </div>
                      <time
                    ...
                    <dl className="mt-3 flex flex-wrap gap-2 text-xs font-semibold">
                      ...
                      <div className="rounded border border-slate-300 bg-white px-2 py-1">
                        ...
                        <dd className="inline text-slate-800">
                          {message.deliveredAt ?? "not recorded"}
                        </dd>
                      </div>
                      <div className="rounded border border-slate-300 bg-white px-2 py-1">
                        <dt className="inline text-slate-500">Failed: </dt>
                        <dd className="inline text-slate-800">
                          {message.failedAt ?? "not recorded"}
                        </dd>
                      </div>
                      <div className="rounded border border-slate-300 bg-white px-2 py-1">
        ...
        <section className="rounded border border-slate-200 bg-white p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            This page only reads and mutates local campaign records through
            existing APIs. It does not send SMS, call providers, run workers,
            create billing records, call live AI, expose secrets, or enable live
            messaging.
          </p>
        </section>
      </div>
