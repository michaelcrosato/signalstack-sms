...
export function LiveTestSmsForm({
  ...
  const [state, setState] = useState<SendState>({
    status: "idle",
    message: "No live test SMS sent from this page yet.",
  });
  ...
  async function submitLiveTestSms(event: FormEvent<HTMLFormElement>) {
    ...
    setState({
      status: "sending",
      message: "Sending live test SMS through Twilio.",
    });
    ...
    <section className="rounded border border-slate-200 bg-white p-5">
      <div className="flex flex-col gap-2">
        ...
        <h2 className="text-xl font-semibold text-slate-950">
          Send One Investor Demo Text
        </h2>
        <p className="text-sm leading-6 text-slate-700">
          This is the only live-send surface. It requires explicit Twilio
          environment credentials, a recipient allowlist, live messaging, and
          the confirmation phrase before it calls Twilio.
        </p>
      </div>
