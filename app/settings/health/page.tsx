    ...
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-8 px-6 py-10">
      <header className="flex flex-col gap-3 border-b border-slate-200 pb-6">
        ...
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-700">
            Read-only local health checkpoint for demo-safe defaults, runtime blockers, and the existing health endpoint.
            This page does not execute checks, call APIs, mutate records, expose secrets, call providers, bill, notify,
        ...
        <Panel title="Health Signals">
          <ul className="grid gap-3 text-sm">
            {healthSignals.map((signal) => (
              <li key={signal.name} className="grid gap-2 border-b border-slate-100 pb-3 md:grid-cols-[10rem_12rem_1fr]">
                <span className="font-semibold text-slate-950">{signal.name}</span>
                <span className="break-words font-mono text-xs text-slate-700">{signal.value}</span>
      ...
      <Panel title="Safety Boundary">
        <ul className="grid gap-2 text-sm text-slate-700">
          <li>No health probes, API requests, commands, scripts, migrations, tests, browser sessions, or git operations are executed from this view.</li>
          ...
          <li>No raw environment values, `.env.local` contents, credentials, token fingerprints, provider verification results, logs, or secret-like values are displayed.</li>
        </ul>
      </Panel>
    </main>
  );
