...
export default async function ReadinessAuditOperationsPage({
  searchParams,
}: ReadinessAuditPageProps) {
  ...
  const auditEvents = await listLiveReadinessAuditEvents(
    currentOrg.orgId,
    query.limit,
    {
      action: query.action,
  ...
  const exportHref = buildReadinessAuditExportHref({
    action: query.action,
    subjectType: query.subjectType,
  });
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-8 px-6 py-10">
      <header className="flex flex-col gap-3 border-b border-slate-200 pb-6">
        ...
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-700">
            Read-only local go-live readiness audit review for{" "}
            {currentOrg.orgName}. This page lists tenant-scoped configuration
            history and CSV links without mutating records, exposing secrets,
            calling providers, sending notifications, creating billing records,
      ...
      <section className="grid gap-3 md:grid-cols-4">
        ...
        <Metric
          ...
          value={readinessAuditStatus.commandExecution}
        />
        <Metric
          label="External impact"
          value={readinessAuditStatus.externalImpact}
        />
        <Metric label="Mutation" value={readinessAuditStatus.mutation} />
        <Metric
          label="Secrets displayed"
          value={String(readinessAuditStatus.secretsDisplayed)}
        />
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_1fr]">
        <Panel title="Action Filters">
          <nav
            aria-label="Readiness audit action filters"
            className="flex flex-wrap gap-2"
          >
            <FilterLink
              href={
                query.subjectType
                  ? `/settings/readiness-audit?subjectType=${query.subjectType}`
        ...
        <Panel title="Subject Filters">
          <nav
            aria-label="Readiness audit subject filters"
            className="flex flex-wrap gap-2"
          >
            <FilterLink
              href={
                query.action
                  ? `/settings/readiness-audit?action=${query.action}`
                  : "/settings/readiness-audit"
              }
              label="All"
              active={!query.subjectType}
            />
      ...
      <Panel title="Audit Events">
        <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <p className="text-sm text-slate-600">
            Local metadata only. Raw credentials, token fingerprints, and
            provider verification results are not shown.
          </p>
        ...
        <ul className="grid gap-3 text-sm">
          {auditEvents.length > 0 ? (
            auditEvents.map((event) => (
              <li
                key={event.id}
                className="grid gap-2 border-b border-slate-100 pb-3 lg:grid-cols-[1fr_180px_180px]"
              >
                <div>
                  ...
                  <p className="mt-1 break-all text-slate-600">
                    Subject ID: {event.subjectId ?? "not recorded"}
                  </p>
                  <p className="mt-1 break-all text-slate-600">
            ...
            <li className="text-slate-600">
              No readiness audit events match the selected filters.
            </li>
          )}
        </ul>
      </Panel>
