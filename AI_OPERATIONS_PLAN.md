...
| App hosting / QA surface | **Docker** (`docker-compose.yml`) — containerized Next.js app + Redis; preview/staging deployment per PR + persistent staging deployment from `develop` | A non-technical human QAs by clicking a URL, never by running anything. |
| Database | **PostgreSQL** (Prisma ORM) plus **Redis** (BullMQ worker queues) — one production project + one development/staging project; migrations applied by agents via Prisma CLI in CI | Keeps destructive experimentation off production. Ephemeral database preview branches (or schema-isolated test databases) isolate e2e runs from the shared dev project (§7.1). |
...
- **Allow:** `npm` scripts, `Node.js`/`tsx`, `tsc`, `Biome`, `Vitest`, `Prisma` CLI (dev instances only), `git` (add/commit/push/branch/fetch/pull), `gh pr` equivalents via the GitHub integration, file edits repo-wide.
