# AI Operations Plan — signalstack-sms
...
| Code host & gate | **GitHub** (`michaelcrosato/signalstack-sms`) with branch protection + **claude-code-action v1 (GA)** for `@claude` mentions and CI autofix | PRs are the unit of review; CI is the objective referee; `@claude` lets the operator turn a plain-English comment into a fix without opening anything else. |
| App hosting / QA surface | **Vercel** — automatic preview deployment per PR + persistent staging deployment from `develop` | A non-technical human QAs by clicking a URL, never by running anything. |
| Database | **Supabase (Postgres)** — one production project + one development/staging project; migrations applied by agents via Prisma CLI in CI | Keeps destructive experimentation off production. Ephemeral database preview branches (or schema-isolated test databases) isolate e2e runs from the shared dev project (§7.1). |
| E2E verification | **Playwright** driven by agents ("test like a human user") against seeded data | Anthropic's harness research found agents skip end-to-end verification unless given browser tooling and told to use it; this closed the biggest quality gap in their long-running trials. |
...
- **Allow:** `npm` scripts, `node`, `tsc`, `biome`, `vitest`/`jest`, `prisma` CLI (dev instances only), `git` (add/commit/push/branch/fetch/pull), `gh pr` equivalents via the GitHub integration, file edits repo-wide.
