...
- Unit tests must verify that `Reflect.apply` invocations of direct, inline cloned, cloned-alias, detached, destructured, direct bound, cloned bound, and aliased bound standard `Request` body readers, including optional `Reflect?.apply(...)` and `Reflect.apply?.(...)` call forms, are treated as body parsing before mutating-route role-gate ordering checks.
- Unit tests must verify that `Reflect.get` standard `Request` body-reader lookups, including direct request, inline cloned request, cloned request alias, declared alias, assigned alias forms, quoted or plain template-literal body-reader property names, local string-literal body-reader property aliases, optional `Reflect?.get` or `Reflect.get?.` call forms, and the optional third receiver argument, are treated as body parsing before mutating-route role-gate ordering checks.
