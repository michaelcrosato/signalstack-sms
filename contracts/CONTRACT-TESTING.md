...
- Unit tests must verify that `/settings/queue` uses static validated metadata for supported local worker command references, safety-boundary copy, public fields, frozen snapshots, stable order, unique identifiers, package-script references, secret-like literal rejection, no command execution, no external impact, and no secret display before local queue metadata renders.
