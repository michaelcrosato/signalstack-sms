...
- Unit tests must verify that the shared operations-index inventory keeps all grouped local operator surfaces on app routes, has no duplicate routes, and includes current safety-sensitive surfaces such as release, health, environment, provider, readiness-audit, notification, and security operations.
