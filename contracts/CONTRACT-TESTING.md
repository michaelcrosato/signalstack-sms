...
- Unit tests must verify that local mutating API handlers call standard `Request` body readers (`json`, `formData`, `text`, `arrayBuffer`, or `blob`) only after their own `requireApiRole` call, including cloned request readers such as `request.clone().json()`, parenthesized cloned readers such as `(req.clone()).json()`, aliased cloned readers such as `const cloned = request.clone(); await cloned.json()`, parenthesized clone aliases such as `const cloned = (req.clone()); await cloned.text()`, semicolonless cloned aliases such as `const cloned = req.clone()` followed by `await cloned.blob()`, typed cloned aliases such as `const cloned: Request = request.clone(); await cloned.text()`, bracket-notation readers such as `request["json"]()` or `request["clone"]()["text"]()`, optional-chained readers such as `request?.json()` or `request?.clone()?.text()`, destructured readers such as `const { json } = request; await json.call(request)`, destructured cloned readers such as `const { json } = request.clone(); await json.call(request)`, detached reader aliases such as `const readJson = request.json; await readJson.call(request)`, destructured readers from cloned request aliases such as `const cloned = request.clone(); const { text } = cloned; await text.call(cloned)`, comment, string, or template-literal mentions of `requireApiRole` or request body readers before body parsing, and handlers that name the `Request` parameter something other than `request`, except signed Twilio webhook handlers.
