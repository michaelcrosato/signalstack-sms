...
- Unit tests must verify that every implemented local mutating API handler contains its own `requireApiRole` call, including exported function and exported const handler forms, except the Twilio inbound/status webhook handlers, which must remain limited to signed webhook validation.
