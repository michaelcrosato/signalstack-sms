...
- Unit tests must verify that every implemented local mutating API route uses `requireApiRole`, except the Twilio inbound/status webhook handlers, which must remain limited to signed webhook validation.
