...
- Unit tests must verify that local mutating API handlers call standard `Request` body readers (`json`, `formData`, `text`, `arrayBuffer`, or `blob`) only after their own `requireApiRole` call, except signed Twilio webhook handlers.
