...
- The seeded product demo path must verify `/dashboard/contacts/:contactId` renders the owner-facing contact detail/edit workflow and updates local profile, consent, notes, tags, list metadata, soft archive, and restore state without SMS, provider calls, billing, live AI, secrets, hard deletion, consent/preflight bypasses, or live feature enablement.
