...
- `docs/PRODUCTION_WORKER_POLICY.md` must remain planning-only until future executable worker gates are added; current validation must continue to prove production-like worker execution and non-`local-demo` worker deployment classes are blocked, including malformed runtime deployment-class values and the reserved `production-live-campaign` planning label. Live-worker control tests must reject malformed, nullish, non-array, sparse, accessor-slot, non-enumerable-index-slot, hidden-required-field, reordered-key proxy, proxy-backed, decorated-array, array-subclass, mutable, non-frozen-descriptor, missing, reordered, renamed, requirement-replaced, unsupported-status, extra-field, accessor-backed/getter-backed, prototype-backed, null-prototype, class-instance, and partial custom control arrays before any future authorization can pass, must prove unsupported and malformed deployment classes do not inspect supplied control evidence, must reject malformed, decorated, missing-field, hidden-required-field, reordered-field, reordered-key, nullish-controls, mutable-wrapper, extensible-wrapper, mutable-field, configurable-field, extra-field, hidden-field, symbol-field, inherited-field, inherited-extra-field, null-prototype, accessor-backed, descriptor-trap, frozen-state-trap, and class-instance authorization wrapper input without executing accessor-backed fields or letting reflection traps escape, must prove malformed wrapper shapes deny before supplied controls are inspected, and must avoid executing getter-backed control fields or letting array, entry, or wrapper proxy reflection traps escape.
