...
- Unit tests must verify that local mutating API handlers call standard `Request` body readers (`json`, `formData`, `text`, `arrayBuffer`, or `blob`) only after their own `requireApiRole` call, including cloned request readers such as `request.clone().json()` and aliased cloned readers such as `const cloned = request.clone(); await cloned.json()`, except signed Twilio webhook handlers.
