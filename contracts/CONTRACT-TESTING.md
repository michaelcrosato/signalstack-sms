...
- Unit tests must verify that the static notification operations inventory exports the supported channel-name vocabulary, keeps channel names and status values inside the supported local notification vocabulary, channel-specific boundary terms, channel boundaries, no-send controls, safety-boundary copy, public fields, frozen snapshots, stable order, unique identifiers, no command execution, no external impact, no secret display, required no-send control terms, whitespace-clean copy, command-like literal rejection, and secret-like literal rejection stable before `/settings/notifications` renders them.
