...
Built-in and object-shaped deployment-class impostors, including maps, sets, weak collections, boxed primitives including proxy-backed and revoked proxy-backed boxed primitive evidence, array buffers, shared array buffers when runtime-supported, every runtime-supported typed-array constructor including proxy-backed and revoked proxy-backed typed-array evidence, data views, promises, regular expressions, errors, URL-shaped records, weak references, finalization registries, runtime-supported Web-platform records, runtime-supported WebAssembly records, runtime-supported Web Crypto records, inherited-coercion-hook records, proxy-backed and revoked proxy-backed runtime-supported platform records, and proxy-backed or revoked proxy object values, must deny before supplied controls are inspected and without invoking `Symbol.toPrimitive`, `toString`, or `valueOf`.
