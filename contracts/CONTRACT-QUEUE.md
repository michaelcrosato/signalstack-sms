...
Built-in object-shaped deployment-class impostors, including maps, sets, weak collections, array buffers, shared array buffers when runtime-supported, every runtime-supported typed-array constructor, data views, promises, regular expressions, errors, URL-shaped records, weak references, finalization registries, runtime-supported Web-platform records, runtime-supported WebAssembly records, runtime-supported Web Crypto records, and proxy-backed or revoked proxy object values, must deny before supplied controls are inspected.
...
Every runtime-supported typed-array constructor must deny as deployment-class evidence, direct controls evidence, and as an authorization-wrapper impostor before the reserved worker class can authorize.
