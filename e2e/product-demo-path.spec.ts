...
test("product dashboard renders seeded owner workflow checkpoints", async ({ page }) => {
  ...
  await expect(nextSteps.getByRole("link", { name: /Review delivery evidence/ })).toHaveAttribute(
    "href",
    "/dashboard/analytics"
  );
  ...
  await expect(analytics.getByText("Delivery evidence", { exact: true })).toBeVisible();
  ...
  await expect(analytics.getByText("Delivery review")).toBeVisible();
  await expect(analytics.getByText(/(No outbound evidence|pending; awaiting provider status|failed; review evidence|All delivered)/).first()).toBeVisible();
  await expect(analytics.getByText("Last delivery evidence", { exact: true })).toBeVisible();
  await expect(analytics.getByText(/(none|\d{4}-\d{2}-\d{2}T)/).first()).toBeVisible();
