...
- No blocker from polishing campaign-detail recipient block-reason copy and delivery snapshot boundary text. Validation passed with `npm run test -- tests/unit/product/campaigns.test.ts`, `npm run typecheck`, `npm run test:e2e:product-demo -- --grep "product campaign detail"`, and the protected `.\scripts\local-gate.ps1`.
