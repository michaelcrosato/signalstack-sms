...
- No blocker from making campaign-detail delivery metrics aggregate all outbound local campaign messages separately from the recent visible delivery rows. Validation passed with `npm run test -- tests/unit/product/campaigns.test.ts`, `npm run typecheck`, `npm run test:e2e:product-demo -- --grep "product campaign detail"`, and the protected `.\scripts\local-gate.ps1`.
