...
- `/settings/notifications` is a read-only local notification operations view. It displays email, in-app, SMS alert, and webhook notification boundaries, no-send controls, runtime gate status, provider status, production override status, and future notification-provider gate requirements; it does not create recipients, subscriptions, templates, jobs, sends, alerts, webhooks, provider calls, billing records, live AI calls, notifications, SMS, email, mutations, expose secrets, or enable live features.
