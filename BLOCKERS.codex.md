...
- No blocker from adding aggregate recipient-readiness metrics to campaign detail. Focused validation passed with `npm run test -- tests/unit/product/campaigns.test.ts` and `npm run test:e2e:product-demo`; the protected local gate passed with `.\scripts\local-gate.ps1`.
