...
The root launch dashboard, `/demo` console navigation, `/settings/demo` readiness links, `/settings/runbook` admin links, `/settings` go-live readiness navigation, `/settings/reports`, `/settings/workflows`, `/settings/releases`, `/settings/integrations`, `/settings/security`, `/settings/environment`, `/settings/health`, `/settings/contracts`, `/settings/validation`, `/settings/exports`, `/settings/webhooks`, `/settings/delivery`, and `/settings/team` link projections are derived from the same shared local operator surface inventory, with unit coverage for launch coverage, demo console coverage, demo operations coverage, reporting/workflow/release coverage, integration/security coverage, environment/health/contract/validation coverage, admin-export coverage, webhook/delivery/team coverage, label alignment, current-page exclusion, and backing app pages. The root Playwright smoke test plus seeded demo-console, demo-operations, reporting, workflow, release, integration, security, environment, health, contract, validation, admin-export, webhook, delivery, team, go-live readiness, and operations-index browser checks also read that shared inventory for visible link coverage instead of maintaining separate route lists.
