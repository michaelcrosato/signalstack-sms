...
The operations index route inventory is shared with unit tests and seeded browser coverage so local surface counts, backing app pages, implemented operator pages, duplicate routes, duplicate labels/notes, stable group/route order, stable projection route order, stable label/note copy shape, rich projection labels/routes/copy boundaries, visible labels/routes, and safety-sensitive links fail fast before green handoff.
