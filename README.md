...
The product workspace starts at `/dashboard`. Deeper product routes now include `/dashboard/contacts` for local audience review/import, `/dashboard/campaigns` for draft composition, recipient selection, compliance preflight, and local scheduling, and `/dashboard/inbox` for local conversation review, demo inbound replies, notes, assignment, and resolve/reopen workflow through existing APIs without SMS, provider, billing, or live-AI side effects.
