...
Notification operations review is available at `/settings/notifications`. Its channel boundaries, no-send controls, safety-boundary copy, public fields, frozen snapshots, stable order, unique identifiers, and secret-like literal guard are validated static metadata before render; it does not create recipients, templates, jobs, sends, alerts, webhooks, provider calls, billing records, live AI calls, notifications, SMS, email, mutations, secrets, or live feature enablement.
