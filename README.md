...
The product workspace starts at `/dashboard`. Deeper product routes now include `/dashboard/contacts` for local audience review/import and `/dashboard/campaigns` for draft composition, recipient selection, compliance preflight, and local scheduling through existing APIs without SMS, provider, billing, or live-AI side effects.
...
Seeded product dashboard, contacts, and campaign path:
