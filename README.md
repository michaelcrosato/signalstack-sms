...
The operations index route inventory is shared with unit tests and seeded browser coverage so local surface counts, backing app pages, implemented operator pages, duplicate routes, duplicate labels/notes, route-aligned labels/notes, whitespace-clean inventory copy, concise action-neutral navigation copy, stable group/route order, stable projection route order, full supplied-inventory copy derivation, supplied-inventory route omission handling, missing-route projection failures, focused projection reachability, stable label/note copy shape, rich projection labels/routes/copy boundaries, visible labels/routes, and safety-sensitive links fail fast before green handoff.
