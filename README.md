...
The root launch dashboard, `/demo` console navigation, `/settings/runbook` admin links, and `/settings` go-live readiness navigation are projected from the same shared local operator surface inventory, with unit coverage for launch coverage, demo console coverage, label alignment, current-page exclusion, and backing app pages. The root Playwright smoke test plus seeded demo-console and operations-index browser checks also read that shared inventory for visible link coverage instead of maintaining separate route lists.
