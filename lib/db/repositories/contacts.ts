...
async function syncContactLabels(
  ...
  const uTagNames = uniqueNames(tagNames);
  if (uTagNames.length > 0) {
    await tx.tag.createMany({
      data: uTagNames.map((name) => ({ orgId, name })),
      skipDuplicates: true
    });
    const tags = await tx.tag.findMany({
      where: { orgId, name: { in: uTagNames } },
      select: { id: true }
    });
    await tx.contactTag.createMany({
      data: tags.map((t) => ({ orgId, contactId, tagId: t.id })),
      skipDuplicates: true
    });
  }

  const uListNames = uniqueNames(listNames);
  if (uListNames.length > 0) {
    await tx.contactList.createMany({
      data: uListNames.map((name) => ({ orgId, name })),
      skipDuplicates: true
    });
    const lists = await tx.contactList.findMany({
      where: { orgId, name: { in: uListNames } },
      select: { id: true }
    });
    await tx.contactListMember.createMany({
      data: lists.map((l) => ({ orgId, contactId, listId: l.id })),
      skipDuplicates: true
    });
  }
}
