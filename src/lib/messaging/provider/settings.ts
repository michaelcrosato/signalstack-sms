...
export function getProviderSettings(input: ProviderSettingsInput) {
  ...
  return {
    ...
    twilio: {
      ...
      accountSidRedacted: input.providerCredential?.accountSidRedacted || (input.env.TWILIO_ACCOUNT_SID ? redactSecret(input.env.TWILIO_ACCOUNT_SID) : null),
      fromNumberRedacted: input.providerCredential?.fromNumberRedacted || (input.env.TWILIO_FROM_NUMBER ? redactSecret(input.env.TWILIO_FROM_NUMBER) : null)
    },
    compliance: {
