...
import { getMessagingProvider, type MessagingProviderName } from "@/lib/messaging/provider";
...
export async function processInboundKeywordsAndAutoReply(
  ...
  if (keywordAction === "OPT_OUT") {
    ...
    const providerName = (process.env.MESSAGING_PROVIDER ?? "dummy") as MessagingProviderName;
    const provider = getMessagingProvider(providerName);
