  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-5xl flex-col gap-8 px-6 py-10">
      ...
      <section className="rounded border border-slate-200 bg-white p-5">
        ...
        <nav
          aria-label="Admin export operations"
          className="mt-4 grid gap-2 md:grid-cols-3"
        >
          {operationLinks.map((link) => (
            <Link
              key={link.href}
              className="rounded border border-slate-200 p-3 text-sm font-medium text-teal-700"
              href={link.href}
            >
  ...
  return (
    <section className="rounded border border-slate-200 bg-white p-5">
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        <Link
          className="inline-flex rounded border border-teal-700 px-3 py-2 text-sm font-semibold text-teal-700"
          href={href}
        >
          Export CSV
        </Link>
        {detailHref ? (
          <Link
            className="inline-flex rounded border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700"
            href={detailHref}
          >
            Review Events
          </Link>
