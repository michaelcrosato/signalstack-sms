  ...
  return (
    <form className="grid gap-5" onSubmit={onSubmit}>
      ...
      <label className="grid gap-2 text-xs font-semibold text-slate-300">
        Message body
        <textarea
          className="min-h-44 rounded-xl border border-white/10 bg-slate-900/60 px-3.5 py-2.5 text-sm text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition"
          disabled={pending}
      ...
      <div className="flex flex-wrap items-center gap-4 border-t border-white/5 pt-4">
        ...
        <p className="text-xs text-slate-400">Local template edit only. No provider calls, SMS, live AI, or billing actions run.</p>
      </div>
      {status ? (
